...
class MontyLexicalRuleParser(object):
    lexicalrules_filename='LEXICALRULEFILE.MDF'
    lex_rules=[]
    rule_names=['char','hassuf','deletesuf','addsuf','haspref','deletepref',\
	'addpref','goodleft','goodright']
    rule_names +=['fchar','fhassuf','fdeletesuf','faddsuf','fhaspref',\
	'fdeletepref','faddpref','fgoodleft','fgoodright']

    def __init__(self,LexiconHandle):
        self.theLexicon=LexiconHandle
        self.lexicalrules_filename=MontyUtils.MontyUtils().find_file(\
		self.lexicalrules_filename)
