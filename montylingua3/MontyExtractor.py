...
class MontyExtractor(object):
    ...
    def extract_info(self,chunked_text,lemmatise_function_handle=None):
        ...
        the_parser_cleaned=map(lambda output_p:output_p[0],filter(\
		lambda alias_str:alias_str[1]=='AX',id_p))
        groupnames_p=map(lambda output_p:output_p[0],filter(\
		lambda alias_str:alias_str[1]=='NX',id_p))
        ...
        j_str=map(lambda output_p:output_p[0],filter(\
		lambda alias_str:alias_str[1]=='VX',id_p))
        groups_arr=map(lambda alias_str:' '.join(alias_str)\
		,factor1(chunked_text))
        print chunked_text
        ...
        dirname1=map(lambda alias_str:input_str(alias_str,enabled_dict),\
		self.find_verb_arg_structures(chunked_text))
    ...
    def make_parameterized_predicates(self,verbose_verb_arg_structure,\
	lemmatise_function_handle=None):
        ...
        alias1=map(lambda output_p:output_p.split('/')[0],filter(\
		lambda alias_str:alias_str.split('/')[1]in['IN','TO'],names_arr.split()))

        if len(alias1)>0:
            inputs.append('prep='+alias1[0])
        cksum_p=map(lambda output_p:output_p.split('/')[0],filter(\
		lambda alias_str:alias_str.split('/')[1]in['DT','CD','PRP$'],\
		names_arr.split()))
        ...
        for cp_arr in range(len(output_pu)):
            alias1=map(lambda output_p:output_p.split('/')[0],filter(\
			lambda alias_str:alias_str.split('/')[1]in['IN','TO'],\
			output_pu[cp_arr].split()))
        ...
        if lemmatise_function_handle==None:
            ...
            if chroot.lower()!=cp_cleaned(table_arr).lower()and chroot.lower()\
			not in['be','have','do']:

                if len(chroot)>3 and len(table_arr)>3 and table_arr.lower().split('/')\
				[0][-1]=='s' and chroot.lower()[-1]!='s':
                    _hugo_p=0
            ...
            if _hugo_p:
                ...
                for cron in map(lambda alias_str:alias_str.lower().split('/')[0],\
				cron_cleaned.split()):
    ...
    def make_concise_verb_arg_structure(self,verbose_verb_arg_structure,\
	lemmatise_function_handle=None):
        hostnames_cleanedt=self.filter_by_tag
        cp_cleaned=self.strip_tags
        hostnamess=self.strip_tags_lemmatised
        ...
        if lemmatise_function_handle==None:
            ...
            names_arr=hostnamess(names_arr)
            output_pu=map(hostnamess,output_pu)
        tmp1='("'+chroot+'" "'+names_arr+'" '+' '.join(map(\
		lambda alias_str:'"'+alias_str+'"',output_pu))+')'
        return tmp1
    ...
    def jist_verb_chunk(self,verbchunk):
        ...
        env_cleaned=filter(lambda alias_str:alias_str[0]=='not' or \
		alias_str[1]not in['MD','RB','TO'],env_cleaned)
    ...
    def _find_linked_objects(self,toks,vc_end_index,prev_obj=''):
        ...
        elif prev_obj in['']and len(iter)>=1 and('/' in toks[iter[0]])and \
		toks[iter[0]].split('/')[1]not in['IN','TO']:
            factor_arr=[]
            ...
            for cp_arr in iter:
                ...
                factor_arr.append(toks[cp_arr])
            output_pu.append(' '.join(factor_arr))
        elif prev_obj in['','NP','PP']and len(iter)>=3 and('/' in toks[iter[0]])and\
		toks[iter[0]].split('/')[1]in['IN','TO']and toks[iter[1]]=='(NX':
            factor_arr=[]
        ...
        elif len(iter)>=4 and('/' in toks[iter[0]])and toks[iter[0]].split('/')[1]\
		in['IN','TO']and('/' in toks[iter[1]])and toks[iter[1]].split('/')[1]in\
		['IN','TO']and toks[iter[2]]=='(NX':
            factor_arr=[]
            factor_arr.append(toks[iter[0]])
        ...
        for cp_arr in range(len(output_pu)):
            gawk_dict=['JJ','JJS','JJR','NN','NNS','NNP','NNPS','VBG','CD',\
			'PRP','PRP$','EX','SYM','WP','WP$','WDT']
    ...
    def extract_pos(self,tagged_text,pos_whitelist,white_or_blacklist='white'):
        ...
        env_cleaned=filter(lambda alias_str:alias_str not in['(NX','NX)',\
		'(VX','VX)'],env_cleaned)
        env_cleaned=map(lambda alias_str:alias_str.split('/'),env_cleaned)
        dirname_arr=pos_whitelist

        if white_or_blacklist.lower()=='black':
            env_cleaned=filter(lambda alias_str:alias_str[1]not in \
			dirname_arr,env_cleaned)
        else :
            env_cleaned=filter(lambda alias_str:alias_str[1]in \
			dirname_arr,env_cleaned)
    ...
    def extract_phrases(self,chunked_text,only_salient_words_p=1):
        ...
        for more in range(len(the_tokenizer1)):
            ...
            hostname_arrd=map(lambda alias_str:alias_str.split('/'),\
			hostname_cleaned[0].split())
            hostname_arrd=filter(lambda alias_str:alias_str[1]not in[\
			'DT','MD'],hostname_arrd)
            hostname_arrd=filter(lambda alias_str:alias_str[0].lower()\
			not in table_str,hostname_arrd)
            env_str=map(lambda alias_str:alias_str[0],hostname_arrd)
            popd=' '.join(env_str).lower().strip()

            if popd=='':
                continue
            hostname_arrd=map(lambda alias_str:alias_str[0]+'/'+\
			alias_str[1],hostname_arrd)
