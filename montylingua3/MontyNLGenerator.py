...
class MontyNLGenerator(object):
    ...
    def __init__(self):
        ...
        for names in cats:
            ...
            domain_arr=tuple(map(lambda arg_cleaned:tuple(arg_cleaned.split(',')),\
			domain_arr.split(';')))
            domain_arr=map(lambda arg_cleaned:(arg_cleaned[0],hostnames(\
			arg_cleaned[1])),domain_arr)
    ...
    def generate_sentence(self,vsoo,sentence_type='declaration',tense='past',\
	s_dtnum=('',1),o1_dtnum=('',1),o2_dtnum=('',1),o3_dtnum=('',1)):
        chroot_arr=self.conjugate_verb
        chownsq,outputf=vsoo[0:2]
        cron_p=vsoo[2:]
        file_dict=['aboard','about','above','across','after','against','along',\
		'amid','among','anti','around','as','at','before','behind','below',\
        ...
        if sentence_type=='imperative':
            ...
            if tense in['progressive','past_progressive','future']:
                case_pa=' '.join([cron1.split()[0],buffer_dict,' '.join(\
				cron1.split()[1:]),' '.join(gawk_str)])+'?'
            else :
                case_pa=' '.join([buffer_dict,cron1,' '.join(gawk_str)])+'?'
        elif sentence_type in('who','what','when','where','why','how'):

            if tense in['progressive','past_progressive','future']:
                case_pa=' '.join([sentence_type,cron1.split()[0],buffer_dict,\
				' '.join(cron1.split()[1:]),' '.join(gawk_str)])+'?'
            else :
                case_pa=' '.join([sentence_type,buffer_dict,cron1,' '.join(gawk_str)])+'?'
        else :
            case_pa=' '.join([buffer_dict,cron1,' '.join(gawk_str)])+'.'
    ...
    def determine_nounphrase(self,nounphrase,det='',number=1):
        gawk_cleaned=['who','what','when','whom','it','its','his','her','hers',\
		'they','their','us','you','me','them','those','these','he','she','we',\
		'mine','yours','ours','theirs','myself','yourself','himself',\
		'herself','itself','ourselves','yourselves','themselves','oneself','my']
    ...
    def all_egocentric_declarations(self,simple_vp):
        ...
        file_arr.append(('I '+self.conjugate_verb(chownsq,'VBP')+' '+\
		dirname_cleaned).strip())
        file_arr.append(('I '+self.conjugate_verb(chownsq,'VBD')+' '+\
		dirname_cleaned).strip())
        file_arr.append(('I '+self.conjugate_verb(chownsq,'VBD')+' not '+\
		dirname_cleaned).strip())
        file_arr.append(('I am '+self.conjugate_verb(chownsq,'VBG')+' '+\
		dirname_cleaned).strip())
        file_arr.append(('I am not '+self.conjugate_verb(chownsq,'VBG')+' '+\
		dirname_cleaned).strip())
        file_arr.append(('I had '+self.conjugate_verb(chownsq,'VBN')+' '+\
		dirname_cleaned).strip())
        file_arr.append(('I hadn\'t '+self.conjugate_verb(chownsq,'VBN')+' '+\
		dirname_cleaned).strip())
        file_arr.append(('I have '+self.conjugate_verb(chownsq,'VBN')+' '+\
		dirname_cleaned).strip())
        file_arr.append(('I haven\'t '+self.conjugate_verb(chownsq,'VBN')+' '+\
		dirname_cleaned).strip())
        file_arr.append(('I want to '+self.conjugate_verb(chownsq,'VB')+' '+\
		dirname_cleaned).strip())
        file_arr.append(('I don\'t want to '+self.conjugate_verb(chownsq,'VB')+\
		' '+dirname_cleaned).strip())
        file_arr.append(('I wanted to '+self.conjugate_verb(chownsq,'VB')+' '+\
		dirname_cleaned).strip())
        file_arr.append(('I didn\'t want to '+self.conjugate_verb(chownsq,'VB')+\
		' '+dirname_cleaned).strip())
        file_arr.append(('I would '+self.conjugate_verb(chownsq,'VB')+' '+\
		dirname_cleaned).strip())
        file_arr.append(('I would\'nt  '+self.conjugate_verb(chownsq,'VB')+' '+\
		dirname_cleaned).strip())
        file_arr.append(('I can '+self.conjugate_verb(chownsq,'VB')+' '+\
		dirname_cleaned).strip())
        file_arr.append(('I can\'t '+self.conjugate_verb(chownsq,'VB')+' '+\
		dirname_cleaned).strip())
        file_arr.append(('I could '+self.conjugate_verb(chownsq,'VB')+' '+\
		dirname_cleaned).strip())
        file_arr.append(('I couldn\'t '+self.conjugate_verb(chownsq,'VB')+' '+\
		dirname_cleaned).strip())
        file_arr.append(('I may '+self.conjugate_verb(chownsq,'VB')+' '+\
		dirname_cleaned).strip())
        file_arr.append(('I may not '+self.conjugate_verb(chownsq,'VB')+' '+\
		dirname_cleaned).strip())
        file_arr.append(('I might '+self.conjugate_verb(chownsq,'VB')+' '+\
		dirname_cleaned).strip())
        file_arr.append(('I might not '+self.conjugate_verb(chownsq,'VB')+\
		' '+dirname_cleaned).strip())
        file_arr.append(('I must '+self.conjugate_verb(chownsq,'VB')+' '+\
		dirname_cleaned).strip())
        file_arr.append(('I must not '+self.conjugate_verb(chownsq,'VB')+' '+\
		dirname_cleaned).strip())
        file_arr.append(('I ought to '+self.conjugate_verb(chownsq,'VB')+' '+\
		dirname_cleaned).strip())
        file_arr.append(('I ought not to '+self.conjugate_verb(chownsq,'VB')+' '+\
		dirname_cleaned).strip())
        file_arr.append(('I shall '+self.conjugate_verb(chownsq,'VB')+' '+\
		dirname_cleaned).strip())
        file_arr.append(('I shall not '+self.conjugate_verb(chownsq,'VB')+' '+\
		dirname_cleaned).strip())
        file_arr.append(('I should '+self.conjugate_verb(chownsq,'VB')+' '+\
		dirname_cleaned).strip())
        file_arr.append(('I shouldn\'t '+self.conjugate_verb(chownsq,'VB')+' '+d\
		irname_cleaned).strip())
        file_arr.append(('I will '+self.conjugate_verb(chownsq,'VB')+' '+\
		dirname_cleaned).strip())
        file_arr.append(('I won\'t '+self.conjugate_verb(chownsq,'VB')+' '+\
		dirname_cleaned).strip())
        file_arr.append(('I would have '+self.conjugate_verb(chownsq,'VBN')+\
		' '+dirname_cleaned).strip())
        file_arr.append(('I wouldn\'t have '+self.conjugate_verb(chownsq,'VBN')+\
		' '+dirname_cleaned).strip())
        return file_arr
    ...
    def output_morph_dict(self):
        ...
        for output in argsv:
            ...
            domain_arr=';'.join(map(lambda arg_cleaned:arg_cleaned[0]+\
			','+'|'.join(arg_cleaned[1]),domain_arr))
