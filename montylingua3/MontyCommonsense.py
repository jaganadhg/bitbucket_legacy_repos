...
class MontyCommonsense(object):
    ...
    def __init__(self,MontyLemmatiser_handle=None,MontyTagger_handle=None):
        ...
        cat_p=self.setitem
        map(lambda stripped:cat_p(ps1,stripped[0],(stripped[1].split(),\
		stripped[2].split())),map(lambda tmps:tmps.split('|'),\
		filter(lambda tagged_cleaned:tagged_cleaned.strip()!='',\
		zlib.decompress(hash1.read()).split('\n'))))
        self.cssdb=ps1
    ...
    def cs_verify_tagged(self,tagged):
        ...
        if len(filter(lambda tagged_cleaned:tagged_cleaned in \
		hostnames_dict,table_arr))>0:
            return tagged
        groupnames_cleaned=' '.join(map(\
		lambda tagged_cleaned:tagged_cleaned.split('/')[0],cmp1))
                ...
                if buffer_dict in hostnames_dict:
                    ...
                    if ps1.has_key(cron):
                        ...
                        if input_p+1<argss and ress:
                            ...
                            if nice1m.lower()in['the','a','an','some',\
							'every','each','most']:
                    ...
                    if csplit_cleaned:
                        ...
                        break
        info1=' '.join(map(lambda tagged_cleaned:tagged_cleaned[0]+\
		'/'+tagged_cleaned[1],map(lambda tmps:tmps.split('/'),cmp1)))
    ...
    def unpp(self,pp):
        ...
        hash1,input_p=map(lambda tagged_cleaned:int(tagged_cleaned.split(\
		'=')[1]),chgrp_p.pop().split(';')[:2])
    ...
    def build_cs_selection_db(self):
        ...
        while more_dict:
            ...
            if more_dict[:len('(CapableOf')]!='(CapableOf' and \
			more_dict[:len('(CapableOfReceivingAction')]!='(CapableOfReceivingAction':
                more_dict=hash1.readline()
                continue
