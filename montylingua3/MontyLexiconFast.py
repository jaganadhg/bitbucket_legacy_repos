...
class MontyLexiconFast:
    ...
    def __init__(self):
        ...
        if MontyUtils.MontyUtils().find_file(self.fast_lexicon_filename+\
		'_1.MDF')!='':
            ...
            self.lexicon_filename=MontyUtils.MontyUtils().find_file(\
    ...
    def load_fastlexicon(self):
        ...
        res_arr=open(c_p,'r')
        built_in_p,input_arr,chgrp1,cd_arr=map(lambda hostnames:int(\
		hostnames),res_arr.read().split())
        res_arr.close()
        ...
        line1=self.array_fromfile(res_arr,self.word_start_arr,built_in_p,\
		self.java_p,java_code='ws')
        res_arr.close()
        res_arr=open(dirname_dict,'rb')
        self.array_fromfile(res_arr,self.word_end_arr,input_arr,\
		self.java_p,java_code='we')
        res_arr.close()
        res_arr=open(output_cleaned,'rb')
        self.array_fromfile(res_arr,self.pos_start_arr,chgrp1,\
		self.java_p,java_code='ps')
        res_arr.close()
        res_arr=open(pairs_cleaned,'rb')
        self.array_fromfile(res_arr,self.pos_end_arr,cd_arr,\
		self.java_p,java_code='pe')
        res_arr.close()
    ...
    def is_word(self,word,case_sensitivity=0):
        ...
        if case_sensitivity:
            ...
            pathname_cleaned=(a_cleaned(word,None)!=None)or(\
			a_cleaned(word.lower(),None)!=None)or(a_cleaned(word.upper(),None)\
			!=None)or(a_cleaned(b_cleaned,None)!=None)
    ...
    def array_fromfile(self,file_ptr,array_ptr,length,java_p=0,java_code='',\
	endian_order='little'):
        history=4
