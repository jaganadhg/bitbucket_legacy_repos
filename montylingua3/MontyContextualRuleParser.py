...
class MontyContextualRuleParser(object):
    contextualrules_filename='CONTEXTUALRULEFILE.MDF'
    rules=[]
    rule_names=['PREVTAG','NEXTTAG','PREV1OR2TAG','NEXT1OR2TAG',\
	'PREV1OR2OR3TAG','NEXT1OR2OR3TAG','SURROUNDTAG','PREVBIGRAM',\
	'NEXTBIGRAM','PREV2TAG','NEXT2TAG']

    def __init__(self):
        self.contextualrules_filename=MontyUtils.MontyUtils().find_file(\
		self.contextualrules_filename)
        ...
        while (inputs):
            ...
            for the_parser_str in range(len(user_dict)):
                ...
                if b_arr:
                    inputs=1
                    b_arru=1
                    print "DEBUG: POS of word",text_arr[word_index]['word'],\
					"changed from",ps1,"to",text_arr[word_index]['pos']
