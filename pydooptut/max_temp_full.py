...
Guide by Tom White.
This example containd Partitioner and combiner class use with pydoop
@author Jaganadh Gopinadhan
@copyright Apache Licence
"""
import sys
from pydoop.pipes import Mapper, Reducer, Factory, Partitioner, runTask
...
class MaxTempPartitioner(Partitioner):
	"""
	Max temperature partitioner class
	"""

	def __init__(self,context):
		super(MaxTempPartitioner, self).__init__(self)
	
	def partition(self,key,numOfReduces):
		reducer_id = (hash(key) & sys.maxint) % numOfReduces
		return reducer_id

runTask(Factory(MaxTempMapper,MaxTempReucer,partitioner_class=MaxTempPartitioner,\
combiner_class=MaxTempReucer))
